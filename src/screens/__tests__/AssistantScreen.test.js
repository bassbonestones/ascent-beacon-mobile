...
jest.mock("../../components/assistant/SymbolHeader", () => {
  ...
  return function MockSymbolHeader({ label, onBack }) {
    return (
...
jest.mock("../../components/assistant/ValueCard", () => {
  ...
  return function MockValueCard({ value }) {
    return (
...
jest.mock("../../components/assistant/ProposedValueCard", () => {
  const { View, Text } = require("react-native");
  return function MockProposedValueCard({ recommendation }) {
    return (
...
jest.mock("../../components/assistant/InputBar", () => {
  ...
  return function MockInputBar({ inputText, onInputChange, onSend }) {
    return (
...
describe("AssistantScreen", () => {
  const mockRoute = { params: { contextMode: "values" } };
  ...
  it("uses default contextMode when not provided", () => {
    const routeNoParams = { params: undefined };
    const { getByTestId } = render(
