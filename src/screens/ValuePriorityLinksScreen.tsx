...
interface ValuePriorityLinksScreenProps {
  route: RouteProp<RootStackParamList, "ValuePriorityLinks">;
  navigation: NativeStackNavigationProp<RootStackParamList>;
}

export default function ValuePriorityLinksScreen({
  route,
  navigation,
}: ValuePriorityLinksScreenProps): React.ReactElement {
  ...
  if (vpl.loading) {
    return (
      <View
        ...
        accessibilityLabel="Loading priorities"
      >
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} accessibilityLabel="Priorities list">
        <Text style={styles.sectionTitle}>
          Select priorities linked to this value:
        </Text>
        {vpl.priorities.length === 0 ? (
          <View style={styles.emptyState}>
            <Text style={styles.emptyText}>No priorities yet</Text>
            <Text style={styles.emptySubText}>
              Create priorities first to link them to values
            </Text>
          </View>
        ) : (
          vpl.priorities
            .filter((p) => p.active_revision)
            .map((priority) => {
              ...
              return (
                <View
                  ...
                  style={[
                    styles.priorityCard,
                    isChanged && styles.priorityCardChanged,
                  ]}
