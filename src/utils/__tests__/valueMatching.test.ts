...
describe("valueMatching utilities", () => {
  ...
  describe("getActiveRevision", () => {
    it("should return active revision when found", () => {
      ...
      expect(result?.statement).toBe("Active");
    });

    it("should return null when no active_revision_id", () => {
      const value = { revisions: [{ id: "rev1" }] } as unknown as Value;
      const result = getActiveRevision(value);
      expect(result).toBeNull();
    });

    it("should return null when no revisions array", () => {
      const value = { active_revision_id: "rev1" } as unknown as Value;
      const result = getActiveRevision(value);
      expect(result).toBeNull();
    });

    it("should return null for null input", () => {
      expect(getActiveRevision(null as unknown as Value)).toBeNull();
    });
  });
        ...
        revisions: [
          {
            id: "r1",
            value_id: "1",
            statement: "I value family and relationships",
            weight_raw: 1,
        ...
        revisions: [
          {
            id: "r2",
            value_id: "2",
            statement: "Career growth is important",
            weight_raw: 1,
            weight_normalized: 50,
            is_active: true,
            origin: "declared",
        ...
        revisions: [
          {
            id: "r2",
            value_id: "2",
            statement: "Health and wellness matter to me",
            weight_raw: 1,
            weight_normalized: 50,
            is_active: true,
            origin: "declared",
