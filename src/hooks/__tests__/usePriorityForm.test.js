...
describe("usePriorityForm", () => {
  ...
  describe("handleNameChange", () => {
    ...
    it("should trigger validation after debounce for longer titles", async () => {
      api.validatePriority.mockResolvedValueOnce({
        name_valid: true,
        name_feedback: [],
      });
      ...
      await waitFor(() => {
        expect(api.validatePriority).toHaveBeenCalled();
      });
    });
  });
  ...
  describe("loadFromPriority", () => {
    it("should load form data from priority object", () => {
      ...
      const priority = {
        ...
        active_revision: {
          ...
          cadence: "weekly",
          constraints: "time",
          value_links: [{ value_id: "v1" }, { value_id: "v2" }],
        },
      };
    ...
    it("should handle priority without active_revision", () => {
      ...
      // Load null priority
      act(() => {
        result.current.loadFromPriority({ id: "p1", active_revision: null });
      });
