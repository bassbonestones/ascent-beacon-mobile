...
import type { PriorityScope } from "../../types";

interface CreatePriorityStep4Props {
  formData: PriorityFormData;
  onFormDataChange: React.Dispatch<React.SetStateAction<PriorityFormData>>;
  onBack: () => void;
  onNext: () => void;
}
...
export default function CreatePriorityStep4({
  ...
  onBack,
  onNext,
}: CreatePriorityStep4Props): React.ReactElement {
  const [showScopeModal, setShowScopeModal] = useState(false);
  const [showScoreModal, setShowScoreModal] = useState(false);
  const scopeCloseRef = useRef<View>(null);
  const scoreCloseRef = useRef<View>(null);

  useEffect(() => {
    if (showScopeModal && Platform.OS === "web") {
      setTimeout(() => {
        (scopeCloseRef.current as unknown as HTMLElement)?.focus?.();
      }, 100);
    }
  }, [showScopeModal]);

  useEffect(() => {
    if (showScoreModal && Platform.OS === "web") {
      setTimeout(() => {
        (scoreCloseRef.current as unknown as HTMLElement)?.focus?.();
      }, 100);
    }
  }, [showScoreModal]);
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  onFormDataChange((prev) => ({
                    ...prev,
                    scope: scope.value as PriorityScope,
                  }));
                  blurActiveElement();
