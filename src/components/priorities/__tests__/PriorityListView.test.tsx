...
describe("PriorityListView", () => {
  ...
  it("renders priority header", () => {
    ...
    expect(getByTestId("priority-header")).toBeOnTheScreen();
  });

  it("shows empty state when no priorities", () => {
    const { getByText } = render(<PriorityListView {...defaultProps} />);
    expect(getByText("No priorities yet")).toBeOnTheScreen();
    expect(getByText("Create your first priority to get started")).toBeOnTheScreen();
  });
  ...
  it("renders priority cards when priorities exist", () => {
    ...
    expect(getByTestId("priority-card-p1")).toBeOnTheScreen();
    expect(getByTestId("priority-card-p2")).toBeOnTheScreen();
  });
  ...
  it("shows stash toggle with count", () => {
    ...
    expect(getByText("▲ Show Stash (1)")).toBeOnTheScreen();
  });
  ...
  it("shows hide stash text when stash is shown", () => {
    ...
    expect(getByText("▼ Hide Stash")).toBeOnTheScreen();
  });
  ...
  it("shows stashed priorities when showStash is true", () => {
    ...
    expect(getByTestId("priority-card-s1")).toBeOnTheScreen();
    expect(getByText("Stashed 1")).toBeOnTheScreen();
  });
  ...
  it("renders back to dashboard button", () => {
    const { getByText } = render(<PriorityListView {...defaultProps} />);
    expect(getByText("Back to Dashboard")).toBeOnTheScreen();
  });
