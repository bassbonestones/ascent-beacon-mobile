...
describe("apiAuth methods", () => {
  ...
  describe("loginWithGoogle", () => {
    it("should login with Google ID token and save tokens", async () => {
      ...
      expect(auth.saveTokens).toHaveBeenCalledWith("new-access", "new-refresh");
  ...
  describe("devLogin", () => {
    it("should perform dev login and save tokens", async () => {
      ...
      expect(auth.saveTokens).toHaveBeenCalledWith("dev-access", "dev-refresh");
      expect(result).toEqual(mockUser);
    });
  });
  ...
  describe("getCurrentUser", () => {
    ...
    it("should return null on auth error when suppressAuthErrors is used", async () => {
      auth.getRefreshToken.mockResolvedValue(null);
      global.fetch.mockResolvedValueOnce({
        ok: false,
        status: 401,
        text: () => Promise.resolve("{}"),
