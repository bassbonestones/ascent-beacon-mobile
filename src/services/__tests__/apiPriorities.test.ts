...
describe("apiPriorities", () => {
  ...
  describe("createPriority", () => {
    it("should call request with POST method and priority data", async () => {
      ...
      (api.request as jest.Mock).mockResolvedValueOnce({ id: "p1" });
  ...
  describe("validatePriority", () => {
    it("should call request with POST to validate endpoint", async () => {
      const data = { name: "Test", why_statement: "Test why" };
      (api.request as jest.Mock).mockResolvedValueOnce({ valid: true });
  ...
  describe("createPriorityRevision", () => {
    it("should call request with POST to revisions endpoint", async () => {
      const revisionData = {
        title: "Updated",
        why_matters: "Test why matters",
      };
      (api.request as jest.Mock).mockResolvedValueOnce({ id: "rev1" });
  ...
  describe("checkPriorityStatus", () => {
    it("should call request with correct endpoint", async () => {
      (api.request as jest.Mock).mockResolvedValueOnce({ linked_values: [] });
  ...
  describe("getPriorityValueLinks", () => {
    it("should call request with correct endpoint", async () => {
      (api.request as jest.Mock).mockResolvedValueOnce({ links: [] });
